...
* The merge() function merges 2 sequential, sorted partitions within a list and has 4 parameters:
    -> The list of numbers containing the 2 sorted partitions to merge
...
def merge(numbers, i, j, k):
    ...
    left_pos = i                        # Initialize left part ition position
    ...
    # If left partition is empty, add remaining elements in right partition to merged numbers
    while right_pos <= k:
        merged_numbers[merge_pos] = numbers[right_pos]
        right_pos+=1
        merge_pos+=1

    # Copy and merge number back to numbers
